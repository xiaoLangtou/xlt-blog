...
<template>
  <nav class="ml-2.5 max-lg:!hidden" aria-label="breadcrumb">
    <ul class="flex-c h-full">
      <li v-for="(item, index) in breadcrumbItems" :key="item.path" class="flex-c">
        <div
          :class="
            isClickable(item, index)
              ? 'c-p h-7 px-1.5 rounded-md tad-200 hover:bg-active-color hover:text-theme'
              : 'h-7 px-1'
          "
          @click="handleBreadcrumbClick(item, index)"
        >
          <span
            class="block max-w-46 overflow-hidden text-ellipsis whitespace-nowrap text-sm leading-7"
            :class="isLastItem(index) ? 'font-semibold text-g-900' : 'text-g-500'"
        ...
        <ArtSvgIcon
          ...
          class="mx-0.5 text-base text-g-400"
          aria-hidden="true"
        />
      </li>
    </ul>
