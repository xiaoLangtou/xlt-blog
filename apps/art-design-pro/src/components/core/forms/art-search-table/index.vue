<script setup lang="ts">
  ...
  defineOptions({ name: 'ArtSearchTable' })
  ...
  export interface ArtSearchOption {
    label: string
  ...
  export interface ArtSearchField {
    /** 查询参数字段名 */
    prop: string
    /** 表单标签 */
    label: string | (() => VNode) | Component
    /** Element Plus 控件类型，默认 input */
    type?: SearchFieldType
    /** 单个字段标签宽度 */
  ...
  watch(
    () => filterKeys.value.join('|'),
    () => syncInitialValues()
  )

  function getComponent(field: ArtSearchField) {
    return field.render ?? componentMap[field.type ?? 'input']
  }

  function getFieldProps(field: ArtSearchField) {
  ...
  function getFieldStyle(field: ArtSearchField) {
          ...
          <slot :name="field.prop" :field="field" :model-value="query[field.prop]">
            ...
            >
              <template v-if="field.type === 'select' && field.options">
                <ElOption v-for="option in field.options" :key="option.value" v-bind="option" />
              </template>
              <template v-if="field.type === 'checkboxgroup' && field.options">
                <ElCheckbox v-for="option in field.options" :key="option.value" v-bind="option" />
              </template>
              <template v-if="field.type === 'radiogroup' && field.options">
                <ElRadio v-for="option in field.options" :key="option.value" v-bind="option" />
              </template>
