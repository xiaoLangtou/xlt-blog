...
const total = computed(() => groups.value?.reduce((sum, group) => sum + group.articles.length, 0) ?? 0)

useSeoMeta({
  title: '时间轴',
  description: '沿时间线浏览全部文章'
})
...
<template>
  <UContainer class="max-w-6xl py-12">
    <header class="mb-10 border-b border-default/60 pb-6">
      <p class="font-mono text-xs tracking-[0.25em] text-dimmed uppercase mb-3">Timeline</p>
      <h1 class="font-display text-3xl text-highlighted tracking-wide">时间轴</h1>
      <p class="text-sm text-muted mt-2">共 {{ total }} 篇文章，沿时间倒序展开。</p>
    </header>

    <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_18rem] gap-12">
      <section>
        <div v-if="pending" class="space-y-4"><USkeleton v-for="index in 5" :key="index" class="h-8 w-full" /></div>

        <div v-else-if="groups?.length" class="space-y-14">
          <section v-for="group in groups" :key="`${group.year}-${group.month}`" class="relative">
            <span class="absolute -top-6 right-0 font-display text-6xl text-default/6 dark:text-white/4 select-none pointer-events-none tabular-nums" aria-hidden="true">
            ...
            <ul class="space-y-1 border-l border-default/60 pl-5">
              <li v-for="article in group.articles" :key="article.slug" class="relative">
                ...
                >
                  <time class="font-mono text-xs text-dimmed tabular-nums shrink-0">{{ formatDay(article.publishedAt) }}</time>
                  <span class="text-default group-hover:text-primary transition-colors min-w-0 truncate">{{ article.title }}</span>
                </NuxtLink>
              </li>
            </ul>
          </section>
